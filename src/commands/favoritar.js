...
module.exports = {
	...
	async autocomplete(interaction) {
		...
		try {
			...
			// FIX 1: Limitar o número de escolhas a 25 (máximo do Discord)
			// FIX 2: Truncar títulos de animes muito longos para evitar erro da API do Discord
			const choices = jikanResponse.data.slice(0, 10).map(anime => {
				const title = anime.title;
				// Se o título for maior que 100 caracteres, ele é cortado.
	...
	async execute(interaction) {
		...
		try {
			...
			if (!jikanDetailsResponse || !jikanDetailsResponse.data) {
				// FIX 3: Mensagem de erro se torna efêmera
			...
			let episodesToSave = animeData.episodes || 0;
        	...
        	if (animeData.status === 'Currently Airing') {
        	    ...
        	    if (airedEpisodes !== null) {
        	        ...
        	        episodeCountText = `${airedEpisodes} / ${animeData.episodes || '??'}`;
        	    }
        	}
			...
			const embed = new EmbedBuilder()
				.setColor(0x5865F2)
				.setTitle(title)
				.setURL(animeData.url || `https://myanimelist.net/anime/${animeId}`)
                // FIX 4: Verificação para sinopse nula para evitar erros
		    ...
		    console.error('Erro fatal na execução de /favoritar (Jikan):', error);
		
		    // VERIFICAÇÃO: Se a resposta inicial já foi editada ou não
		    if (interaction.deferred || interaction.replied) {
		        // Se já respondemos, usamos followUp para a mensagem de erro efêmera
		        await interaction.followUp({ 
		            content: 'Ocorreu um erro inesperado ao processar sua solicitação.',
		            ephemeral: true 
		        });
		    } else {
		        // Se por algum motivo não houve deferral, respondemos diretamente
